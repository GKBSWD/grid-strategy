...
- **K线数据连续性检查**：保存前自动检测时间戳缺口（含历史数据与新数据衔接、本次数据内部连续性），发现缺失尝试单次补获取；若补全失败则截断到最长连续时段，确保回测数据无时间断层
- **实际数据优先**：若数据获取失败或仅获取到部分数据，系统自动以实际获取到的连续数据进行回测，回测天数、年化收益率等指标均基于实际数据周期计算
...
> **重要说明**：文件名中的结束时间和K线数量始终反映**实际最长连续数据段**的范围。若数据获取存在缺口或部分失败，系统会自动截断到最后一个连续时刻，文件名会据此更新，而非使用用户请求的时间范围。
...
| 起始/结束时间 | 回测时间范围（UTC+8，结束时间建议设为XX:59:00且距离当前时间>1h，避免获取失败；若实际获取数据范围小于请求范围，系统自动以实际可用数据回测） |
...
- **计算公式**: 套利次数 × 网格间距 × 单网格交易数量 - 网格交易手续费
...
- **含义**: 将回测收益折算为年化收益率
- **计算公式**: 收益率 × (365 / 回测天数) × 100%
- **注意**: 回测天数基于**实际获取到的连续K线数据**的时间范围计算，而非用户请求的时间范围。若数据获取不完整，年化收益率会自动调整为基于实际数据周期。

#### 7. **网格套利年化收益率** (**grid_annualized_return**)
- **含义**: 基于纯粹网格套利收益的年化收益率
- **计算公式**: (网格套利利润 / 初始保证金) × (365 / 回测天数) × 100%
- **注意**: 同上，回测天数基于实际连续数据的时间范围计算
